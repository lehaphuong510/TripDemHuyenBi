...
export async function onRequest(context) {
  ...
  try {
    ...
    if (url.pathname === '/api/config' && request.method === 'GET') {
      ...
      if(idxTenDot === -1 || idxGioiHan === -1) throw new Error("Không tìm thấy cột 'Nội dung option' hoặc 'SL giới hạn'.");
...
async function getGoogleAuthToken(clientEmail, privateKey) {
  ...
  // XÓA TRIỆT ĐỂ LỖI ATOB BẰNG WHITELIST VÀ PADDING TỰ ĐỘNG
  let base64Key = privateKey.replace(/-----.*?-----/g, ''); // Bỏ header/footer
  base64Key = base64Key.replace(/[^A-Za-z0-9+/=]/g, '');     // Chỉ giữ lại Base64 thuần túy
  while (base64Key.length % 4 !== 0) {
      base64Key += '=';                                      // Bù dấu = cho chia hết cho 4
  }
